"""008-M-Xianfeng.py - ETL Maritimo Xianfeng. Uso: python 008-M-Xianfeng.py <input.xlsx> <output.xlsx>"""
import sys,os,logging,pandas as pd
logging.basicConfig(level=logging.INFO,format="%(asctime)s - %(levelname)s - %(message)s")
logger=logging.getLogger(__name__)
MAPEO_COLUMNAS={"VAREITY":"Variedad","SIZE":"Calibre","COMMODITY":"Marca","REMARKS":"Envop","QUANTITY":"Cantidad","PRICE":"PrecioUnitario"}
MAPEO_COSTOS={
...
def encontrar_fila_header(ruta):
    ...
    for idx,row in df_temp.iterrows():
        ...
        if "VAREITY" in s and "SIZE" in s and "QUANTITY" in s:return idx
    return 3
...
def main():
    if len(sys.argv)<3:print("Uso: python 008-M-Xianfeng.py <input.xlsx> <output.xlsx>",file=sys.stderr);sys.exit(1)
