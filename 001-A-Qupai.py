"""
001-A-Qupai.py
Transformación ETL - Aéreo - Qupai
Genera un .xlsx con 2 hojas: Precios y Gastos
Uso: python 001-A-Qupai.py <input.xlsx> <output.xlsx>
"""
...
def codificar_variedad(nombre: str) -> str:
    return MAPEO_VARIEDAD.get(nombre.strip().upper(), nombre.strip().upper()[:2])


def codificar_envop(descripcion: str) -> str:
    return MAPEO_ENVOP.get(descripcion.strip().upper(), descripcion.strip().upper()[:6])
...
def main():
    if len(sys.argv) < 3:
        print("Uso: python 001-A-Qupai.py <input.xlsx> <output.xlsx>", file=sys.stderr)
        sys.exit(1)
