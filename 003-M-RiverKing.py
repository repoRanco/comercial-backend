"""
003-M-RiverKing.py
Transformación ETL - Marítimo - RiverKing
Genera un .xlsx con 2 hojas: Precios y Gastos
Uso: python 003-M-RiverKing.py <input.xlsx> <output.xlsx>
"""
...
MAPEO_COLUMNAS = {
    'VARIETY':'Variedad','SIZE':'Calibre','LABEL':'Marca','PACKAGE':'Envop','QTY':'Cantidad','PRICE':'PrecioUnitario'
}
MAPEO_COSTOS = {
    "VAT":(40,"01"),"import VAT":(40,"01"),"Import VAT":(40,"01"),
    "Commission":(40,"02"),"Service fee":(40,"02"),"Commission Fee":(40,"02"),
    "Forwarding Charges":(40,"04"),"import service fee":(40,"04"),"Import Service Fee":(40,"04"),
    "Customs inspection fee":(40,"04"),"Import Service Charges":(40,"04"),
    "Market Charges":(40,"05"),"Market Cost":(40,"05"),"Market   Charge":(40,"05"),
...
def main():
    if len(sys.argv)<3: print("Uso: python 003-M-RiverKing.py <input.xlsx> <output.xlsx>",file=sys.stderr); sys.exit(1)
    ip,op=sys.argv[1],sys.argv[2]
    if not os.path.exists(ip): print(f"No encontrado: {ip}",file=sys.stderr); sys.exit(1)
    df=pd.read_excel(ip,header=encontrar_fila_header(ip))
    ...
    costos=obtener_costos(ip); df_p=generar_precios(filas); df_g=generar_gastos(costos,total_cajas)
