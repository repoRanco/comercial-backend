"""010-M-Qinguo.py - ETL Maritimo Qinguo. Uso: python 010-M-Qinguo.py <input.xlsx> <output.xlsx>"""
import sys,os,logging,pandas as pd
logging.basicConfig(level=logging.INFO,format="%(asctime)s - %(levelname)s - %(message)s")
logger=logging.getLogger(__name__)
MAPEO_COLUMNAS={"(Variety)":"Variedad","(Size)":"Calibre","(Brand)":"Marca","(Package style)":"Envop","(Quantity)":"Cantidad","(Unit Price)":"PrecioUnitario"}
MAPEO_COSTOS={
...
PALABRAS_FIN_DATOS=["1 boxes for random inspection(wastage)"];COLUMNAS_REQUERIDAS=["Variedad","Calibre","Marca","Envop","Cantidad","PrecioUnitario"]
...
def main():
    if len(sys.argv)<3:print("Uso: python 010-M-Qinguo.py <input.xlsx> <output.xlsx>",file=sys.stderr);sys.exit(1)
