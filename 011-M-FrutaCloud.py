"""011-M-FrutaCloud.py - ETL Maritimo FrutaCloud. Uso: python 011-M-FrutaCloud.py <input.xlsx> <output.xlsx>"""
import sys,os,logging,pandas as pd
logging.basicConfig(level=logging.INFO,format="%(asctime)s - %(levelname)s - %(message)s")
logger=logging.getLogger(__name__)
MAPEO_COLUMNAS={"Variety":"Variedad","Size":"Calibre","Label":"Marca","Packing Code":"Envop","Quantity Received":"Cantidad","Sales @(LC)":"PrecioUnitario"}
MAPEO_COSTOS={
    "Tax @(LC)":(40,"01"),
...
PALABRAS_FIN_DATOS=["總計"];COLUMNAS_REQUERIDAS=["Variedad","Calibre","Marca","Envop","Cantidad","PrecioUnitario"]
...
def encontrar_fila_header(ruta):
    ...
    for idx,row in df_temp.iterrows():
        ...
        if "Variety" in s and "Size" in s and "Quantity Received" in s:return idx
    return 3
...
def main():
    if len(sys.argv)<3:print("Uso: python 011-M-FrutaCloud.py <input.xlsx> <output.xlsx>",file=sys.stderr);sys.exit(1)
